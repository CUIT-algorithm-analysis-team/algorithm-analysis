...
class algorithm_analysis:
    ...
    def test_time(self, n, start, step, datastyle):
        ...
        #         yield i
        for i, data in enumerate(data_loader(n, start, step)):
            for sort_func in list(self.sort_algorithms.keys()):
                t0 = time.time()
                result = sort_func(data)
                t1 = time.time()
                self.costed_time[self.sort_algorithms[sort_func]].append(t1 - t0)
                self.question_size[self.sort_algorithms[sort_func]].append(len(data))
            yield i
